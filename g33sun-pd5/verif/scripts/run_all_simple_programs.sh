#!/usr/bin/env bash
# Run test_pd for every *.x simple-program benchmark and collect [W] trace lines.
...
BENCH_DIR="${ROOT_DIR}/../rv32-benchmarks/simple-programs"
...
if (( ${#MEM_FILES[@]} == 0 )); then
  echo "No *.x files found in ${BENCH_DIR}" >&2
  exit 1
fi
